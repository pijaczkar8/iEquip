...
bool justUpdatedWidgetVisibility = false
...
Event OnActorAction(int actionType, Actor akActor, Form source, int slot)
	...
	if akActor == PlayerRef
		...
		elseIf actionType == 7 || actionType == 8 ;Draw Begin, Draw End - In case we're drawing a spell which won't have been caught by Draw Begin
			if justUpdatedWidgetVisibility
				justUpdatedWidgetVisibility = false
			else
				WC.updateWidgetVisibility()
				if actionType == 7 ;To stop it happening on both Draw Begin and Draw End if we're drawing anything other than a spell
					justUpdatedWidgetVisibility = true
				endIf
			endIf
