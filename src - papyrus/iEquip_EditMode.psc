...
function SavePreset(bool bUpdatingPreset = false, string presetName = "")
    ...
    if updateExistingPreset == 1
        ...
        if presetName != ""
            ...
            jMap.setInt(jSavePreset, "DSEnabled", WC.bDropShadowEnabled as int)
            jMap.setFlt(jSavePreset, "DSAlpha", WC.fDropShadowAlpha)
            jMap.setFlt(jSavePreset, "DSAngle", WC.fDropShadowAngle)
            jMap.setInt(jSavePreset, "DSBlur", WC.iDropShadowBlur)
...
function LoadPreset(int jPreset)
    ...
    if jMap.getInt(jPreset, "DSEnabled", -1) != -1
        WC.bDropShadowEnabled = jMap.getInt(jPreset, "DSEnabled") as bool
        if WC.bDropShadowEnabled
            WC.fDropShadowAlpha = jMap.getFlt(jPreset, "DSAlpha")
            WC.fDropShadowAngle = jMap.getFlt(jPreset, "DSAngle")
            WC.iDropShadowBlur = jMap.getInt(jPreset, "DSBlur")
            WC.fDropShadowDistance = jMap.getFlt(jPreset, "DSDistance")
            WC.fDropShadowStrength = jMap.getFlt(jPreset, "DSStrength")
        endIf
    endIf
