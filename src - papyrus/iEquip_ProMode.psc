...
function togglePreselectMode(bool togglingEditModeOrRefreshing = false, bool enablingOnLoad = false)
	...
	if bPreselectEnabled || togglingEditModeOrRefreshing
		...
		if bPreselectMode
			...
			abPreselectSlotEnabled[0] = ((WC.aiCurrentlyPreselected[0] != -1) && JArray.count(WC.aiTargetQ[0]) > 1 && !WC.bPlayerIsMounted) || togglingEditModeOrRefreshing
			...
			abPreselectSlotEnabled[2] = (WC.bShoutEnabled && bShoutPreselectEnabled && WC.aiCurrentlyPreselected[2] != -1 && !WC.bPlayerIsMounted) || togglingEditModeOrRefreshing
...
function quickHeal()
    ...
    elseIf PO.getPotionTypeCount(0) > 0
	    ...
	    bQuickHealActionTaken = true
	elseIf PO.bShowStatFullNotifications
		debug.notification(iEquip_StringExt.LocalizeString("$iEquip_PO_not_noneLeft{"+"$iEquip_PO_restoreHealth"+"}"))
	endIf
